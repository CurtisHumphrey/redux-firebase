...
function convert_value(v, ref) {
  switch (v) {
    case SPECIAL_VALUES.KEY:
      return ref.key;
    case SPECIAL_VALUES.UID:
      return get_uid();
    default:
      return v;
  }
...
function transform_payload(payload, ref) {
  if (_.isObject(payload))
    return _.mapValues(payload, (v) => convert_value(v, ref));
...
handlers.once = ({
  meta: { path, update_action, init_value, batch, sort },
}) => (dispatch, getState) => {
  if (batch)
    return batch_get({
      is_on: false,
...
const batch_check = ({ is_on, path, update_action, batch, sort, dispatch }) => (
  snap
) => {
  let keys = [];
...
handlers.on = ({ meta: { path, update_action, init_value, batch, sort } }) => (
  dispatch,
  getState
) => {
  ...
  const response = new Promise((resolve) => {
    setTimeout(
      // on is not always async this force it to be
      () => {
...
handlers.push = ({ payload, meta: { path } }) => (dispatch, getState) => {
  const this_ref = get_root_ref()
    .child(make_path(path))
    .push();
  this_ref.then(() => {
...
handlers.switch = (action) => (dispatch, getState) => {
  ...
  handlers.off({ meta: { path: action.meta.old_path } })(dispatch, getState);
  return handlers.on(action)(dispatch, getState);
};
